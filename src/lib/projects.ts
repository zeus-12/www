...
export const TECH_STACK: { [key: string]: techStack } = {
  ...
  SWIFT: { title: "Swift", color: COLOR.ORANGE },
  SWIFTUI: { title: "SwiftUI", color: COLOR.CYAN },
  ELECTRON: { title: "Electron", color: COLOR.CYAN },
  TYPESCRIPT: { title: "TypeScript", color: COLOR.BLUE },
  AI_SDK: { title: "AI SDK", color: COLOR.PINK },
};
...
export const PROJECTS_DATA: Project[] = [
  {
    title: "Plan",
    shortDescription:
      "Desktop cockpit for driving coding agents. Comment on any chat, diff or file and send it straight to the live session.",
    description:
      "A desktop app for reviewing and iterating on coding agents without copy-pasting your life away. It reads your local Claude projects and sessions, and lets you select any text in a chat, diff, or file, leave comments on it, and send them all into the live agent session running in an embedded terminal. Also does staged/unstaged diff review down to the hunk level, fuzzy file search, project-wide search, and spins up git worktrees per repo. Drives the terminal CLI rather than an SDK, so it can support any harness.",
    techStack: [TECH_STACK.ELECTRON, TECH_STACK.REACT, TECH_STACK.TYPESCRIPT],
